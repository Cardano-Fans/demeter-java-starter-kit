...
public class TransactionCommands {
    ...
    public void sendAda(@ShellOption(value = {"-f"}, help = "from (address or account name)") String from,
        ...
        writeLn(success("Transaction worked, result: %s", result.getValue()));
    }
    ...
    public void claimAda(
        ...
        val contractTxBuilder = output.outputBuilder()
                ...
                .andThen((context, txn) -> {
                    val evalReedemers = evaluateExUnits(txn, context.getUtxos(), protocolParamsSupplier.getProtocolParams());
    ...
    private static List<Redeemer> evaluateExUnits(Transaction txn,
                                                  Set<Utxo> utxos,
    ...
    // this contract is already deployed on pre-prod and preview networks therefore we just need t
